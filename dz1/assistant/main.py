...
output_interface = ConsoleOutput()
...
def display_text(text):
    output_interface.display_text(text)


def display_error(error):
    output_interface.display_error(error)
...
def sort_folder(param_list):
    ...
    print(
        f'Your files are sorted in the directory {new_dir}, and empty folders have been deleted.')
...
def notes(param_list):
    ...
    while True:
        ...
        if choice == "1":
            ...
            assistant.edit_note(note_id, new_text, [
                                tag.strip() for tag in new_tags])
        elif choice == "5":
...
def main():
    while True:
        source_command = prompt(
            "Enter command: ", completer=MyCompleter(address_book, commands))
        command, param_list = parser(source_command)
        if not command:
            display_text(
                f"YOU ENTERED A WRONG COMMAND!!!\n{help}\nTRY AGAIN!!!")
            continue
        result = handler(command)(param_list)
        if result == 'exit':
            display_text("До свидания!")
            break
        else:
            display_text(result)
            save_data()
